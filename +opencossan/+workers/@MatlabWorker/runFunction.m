...
elseif ispc
    Ssystemcall=[Xmio.Spath(1:2) ' && cd "' Xmio.Spath '" && "'...
        fullfile(OpenCossan.getMatlabPath,'bin','matlab') '" -nosplash -nodesktop -wait -r ' ...
        strrep(Xmio.Sfile,'.m','') ' '];
end
...
if strcmp(Xmio.Sformat,'matrix') || ~strcmp(Xmio.Sformat,'structure')
    Poutput = Moutput;
else
    Poutput = Toutput;
end
