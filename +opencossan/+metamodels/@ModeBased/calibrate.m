...
if exist('Nsamples','var')
    Xobj.XcalibrationInput = sample(Xobj.XFullmodel.Input,'Nsamples',Nsamples);
end
...
if ~isempty(Xobj.XcalibrationInput) && isempty(Xobj.XcalibrationInput.Samples)
            error('openCOSSAN:metamodel:calibration',...
            'XcalibrationInput does not contain any samples');
end

if ~isempty(Xobj.XcalibrationInput)
    if Xobj.XcalibrationInput.Samples.Nsamples~= 0
        Nsamples = Xobj.XcalibrationInput.Samples.Nsamples;
    else
        error('openCOSSAN:metamodel:calibration',...
...
if  isempty(Xobj.XcalibrationOutput)
    ...
    for isim = 1:XSimOut.Nsamples
        Xmodescalibration(isim) = opencossan.common.outputs.Modes('MPhi',XSimOut.TableValues.(Xobj.Cnamesmodalprop{2})(isim),...
                                        'Vlambda',XSimOut.TableValues.(Xobj.Cnamesmodalprop{1})(isim));
    end
