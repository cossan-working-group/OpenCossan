...
Xevaluator=Xobj.XprobabilisticModel.Xevaluator;
% Extract PerformanceFunction and performance function name
% XperformanceFunction=Xevaluator.CXsolvers{1};
% SperfName=Xobj.XprobabilisticModel.SperformanceFunctionVariable;
% Update Input object
...
Xpm=ProbabilisticModel('Xmodel',Xmodel);
...
Xobj.XadaptiveLineSampling.VdirectionSNS=Vdirection/norm(Vdirection); % was Xobj.XadaptiveLineSampling.Valpha
% reliability analysis for the maximum
if Xobj.LuseMCtoFinalise
    Xmc=opencossan.simulations.MonteCarlo('Nsamples',Xobj.NmonteCarloSamples);
    [XpfMAX,~]=Xmc.computeFailureProbability(Xpm);
else
    [XpfMAX,XlineDataMAX]=Xobj.XadaptiveLineSampling.computeFailureProbability(Xpm);
    
%     XlineDataMAX=simulations.LineSamplingOutput('Ladaptivelinesampling',true,...
%         'LdeleteResults',false,...
%         'SmainPath',Xobj.XadaptiveLineSampling.StempPath,...
%         'Vinitialdirectionsns',Xobj.XadaptiveLineSampling.VdirectionSNS,...
%         'Sperformancefunctionname',XpfMAX.SperformanceFunction,...
%         'Xinput',Xinput);
end
%% compute MINIMUM
% Extract Input from Extreme Case object
...
if Xobj.LuseMCtoFinalise
    ...
    [XpfMIN,XlineDataMIN]=Xobj.XadaptiveLineSampling.computeFailureProbability(Xpm);
    
%     XlineDataMIN=simulations.LineSamplingOutput('Ladaptivelinesampling',true,...
%         'LdeleteResults',false,...
%         'Sperformancefunctionname',XpfMIN.SperformanceFunction,...
%         'Xinput',Xinput);
end
...
%         Nevaluations=XlineDataMAX.Nevaluations+XlineDataMIN.Nevaluations;
        Nevaluations=XlineDataMAX.Npoints+XlineDataMIN.Npoints;

    end
    CXresults{1}=opencossan.optimization.Optimum;
    Niterations=0;
else
    if Xobj.LuseMCtoFinalise
        Nevaluations=2*Xobj.NmonteCarloSamples;
    else
        Nevaluations=size(Mfp,1)+XlineDataMAX.Npoints+XlineDataMIN.Npoints;
    end
end
%% Assign the output of the analysis
Xextrema=opencossan.intervals.Extrema('Sdescription','Solution of the Extreme Case analysis',...
