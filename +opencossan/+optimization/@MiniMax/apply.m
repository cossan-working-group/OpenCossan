function optimum = apply(obj, varargin)
    % APPLY This method applies the algorithm MiniMax for multi-objective optimization.
    %
    ...
    options.MaxIters = obj.MaxIterations;
    options.TolCon = obj.ConstraintTolerance;
    options.TolFun = obj.ObjectiveFunctionTolerance;
    options.TolX = obj.DesignVariableTolerance;
    options.FinDiffRelStep = obj.FiniteDifferenceStepSize;
    options.FinDiffType = obj.FiniteDifferenceType;
    ...
    if isempty(optProb.Constraints)
        ...
        [optimalSolution,~,~,exitFlag] = fminimax(hobjfun, ...
        ...
        hconstraints = @(x) evaluate(optProb.Constraints,'optimizationproblem',optProb,...
            'referencepoints',x,'model',memoizedModel, ...
            'scaling',obj.ConstraintScalingFactor);
        
        [optimalSolution, ~, ~, exitFlag]  = fminimax(hobjfun, ...
            Xop.VinitialSolution,[],[],[],[], ...
            Xop.VlowerBounds,Xop.VupperBounds, ...
            hconstraints, ...
            options);
    end
