classdef JobManager
    ...
    methods
        ...
        function Xobj=JobManager(varargin)
            ...
            if isempty(Xobj.Sworkingdirectory)
                % Use defailt working path defined in OpenCossan
                Xobj.Sworkingdirectory=opencossan.OpenCossan.getWorkingPath;
            end
            
            % It is mandatory to have a JobManagerInterface object. If such
            % object is missing, an error is given.
            assert(isa(Xobj.Xjobmanagerinterface,'opencossan.highperformancecomputing.JobManagerInterface'), ...
        ...
        SjobID = submitJob(Xobj,varargin) % submit single job
        CSjobID = submitAnalysis(Xobj,Nsamples) % submit analysis (multiple jobs)
    ...
    methods (Access=private)
        ...
        Sscriptname = prepareGridEngineArrayScript(Xobj)
    end
end
