...
elseif strcmpi(Xobj.Xjobmanagerinterface.Stype,'lsf')
    if isempty(Xobj.Xdependent)
        if ~isempty(Xobj.Sjobname)
            string=[Xobj.Xjobmanagerinterface.SsubmitJob ' -J '  Xobj.Sjobname num2str(NsimulationNumber) ' < ' Sscriptname];
        else
            string=[Xobj.Xjobmanagerinterface.SsubmitJob ' < ' Sscriptname];
        end
    else % a dependent job is present
