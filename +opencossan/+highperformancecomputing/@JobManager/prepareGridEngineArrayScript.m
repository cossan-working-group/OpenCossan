function Sscriptname = prepareGridEngineArrayScript(Xobj,NsimulationNumber)
...
assert(isempty(Serror),'openCOSSAN:JobManager:submitJobArray','Error: %s\n Path: %s', ...
...
fprintf(Nfid,'%s\n',['#$ -e ' Xobj.Sfoldername '$SGE_TASK_ID.err' ]);
% Save std output in the cwd
fprintf(Nfid,'%s\n',['#$ -o ' Xobj.Sfoldername '$SGE_TASK_ID".out']);
...
if ~OpenCossan.hasSSHConnection
    ...
    fprintf(Nfid,'%s\n',['cd ' fullfileunix(Xobj.Sworkingdirectory,Xobj.Sfoldername) '$SGE_TASK_ID";']);
else
    % change directory to the job work folder on the cluster
    fprintf(Nfid,'%s\n',['cd ' fullfileunix(Xssh.SremoteWorkFolder ,Xobj.Sfoldername) '$SGE_TASK_ID";']);
end
...
if ~isempty(Xobj.Sfoldername)
    ...
    if ~OpenCossan.hasSSHConnection
        fprintf(Nfid,'%s\n',['mv $START_DIR/' Xobj.Sfoldername '$SGE_TASK_ID.err ' fullfileunix(Xobj.Sworkingdirectory,Xobj.Sfoldername) '$SGE_TASK_ID']);
        fprintf(Nfid,'%s\n',['mv $START_DIR/' Xobj.Sfoldername '.out ' fullfileunix(Xobj.Sworkingdirectory,Xobj.Sfoldername) '$SGE_TASK_ID']);
    else
        fprintf(Nfid,'%s\n',['mv $START_DIR/' Xobj.Sfoldername '$SGE_TASK_ID.err ' fullfileunix(Xssh.SremoteWorkFolder,Xobj.Sfoldername) '$SGE_TASK_ID']);
        fprintf(Nfid,'%s\n',['mv $START_DIR/' Xobj.Sfoldername '$SGE_TASK_ID.out ' fullfileunix(Xssh.SremoteWorkFolder,Xobj.Sfoldername) '$SGE_TASK_ID']);
    end
end
