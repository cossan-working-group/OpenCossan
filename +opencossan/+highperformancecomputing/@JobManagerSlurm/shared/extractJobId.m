...
if isempty(matchedTokens)
    ...
    jobID = '';
    opencossan.OpenCossan.cossanDisp(sprintf('%s: Failed to extract Job ID from sbatch output: \n\t%s',...
        mfilename, trimmedCommandOutput),1);
else
    jobID = matchedTokens{1};
    opencossan.OpenCossan.cossanDisp(sprintf('%s: Job ID %s was extracted from sbatch output: \n\t%s', ...
        mfilename, jobID, trimmedCommandOutput),1);
end
