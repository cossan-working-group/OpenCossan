...
% EXAMPLE 2:
%
% Supposed your ascii file mydata.log begins with the following lines: 
%
...
%
% type
% A = txt2mat('mydata.log',0,10,'%d');
% or just
% A = txt2mat('mydata.log');
%
% -------------------------------------------------------------------------
%
% EXAMPLE 3:
...
%
% A = txt2mat('mydata.log', nh, nc, cstr, {sr1,sr2}, {'Warng'});
%
%   A =
...
% sr2  = ',.'       % to replace the decimal comma with a dot, and
%
% A = txt2mat('mydata.log',nh,nc, cstr, {sr1,sr2});
%
%   A =
...
% --- E-Mail: -------------------------------------------------------------
% x=-2:3;
% OpenCossan.cossanDisp(char(round([-0.32*x.^5+0.43*x.^4+1.75*x.^3-5.90*x.^2-0.95*x+116,...
...
if valpos(4) ~= 0
    ...
    elseif ischar(varargin{valpos(4)});
        ...
        OpenCossan.cossanDisp([mfilename ': for future versions, please use a single cell array of strings as an input argument for multiple replacement strings.'])
...
if valpos(5) ~= 0 && ~has_sr_input_only     % bad line strings
    ...
    elseif ischar(varargin{valpos(5)});
        kl_input_ca = {varargin{valpos(5)}};
        OpenCossan.cossanDisp([mfilename ': for future versions, please use a single cell array of strings as an input argument for bad line marker strings.'])
    else
...
if ~all([is_argin_num_header, is_argin_num_colon]) %, is_argin_conv_str]); % commented out as so far anatxt's conv_str is only '%f'
    ...
    % display information:
    if infolvl >= 1
        OpenCossan.cossanDisp(repmat('*',1,length(ffn)+2));
        OpenCossan.cossanDisp(['* ' ffn]);
        if numel(ferrmsg)==0
            sr_display_str = '';
            for idx = 1:num_sr;
                sr_display_str = [sr_display_str ' �' sr_input_ca{idx} '�']; %#ok<AGROW>
            end
            OpenCossan.cossanDisp(['* read mode: ' readmode]);
            OpenCossan.cossanDisp(['* ' num2str(num_ali)    ' data lines analysed' ]);
            OpenCossan.cossanDisp(['* ' num2str(num_header) ' header line(s)']);
            OpenCossan.cossanDisp(['* ' num2str(num_colon)  ' data column(s)']);
            OpenCossan.cossanDisp(['* ' num2str(num_sr)     ' string replacement(s)' sr_display_str]);
        else
            OpenCossan.cossanDisp(['* fread error: ' ferrmsg]);
        end
        OpenCossan.cossanDisp(repmat('*',1,length(ffn)+2));
    end % if
        ...
        while ldx_lo < num_lf
            ...
            if ~isempty(errmsg)
                if nextindex <= lf_idc(ldx_hi)-(lf_idc(ldx_lo)+1)+1   
                    ...
                    if infolvl >= 2
                        OpenCossan.cossanDisp(['Sscanf error after reading ' num2str(val_cntr) ' numeric values. Critical row (no. ' num2str(err_line_idx-1) '): '])
                        OpenCossan.cossanDisp(f8(lf_idc(err_line_idx-1)+1:lf_idc(err_line_idx)-1)')
                    end % if
                end % if
                break;
            end
            ...
            if numel(Atmp) ~= num_colon * num_lines_loop 
                ...
                if infolvl >= 2
                    OpenCossan.cossanDisp('Warning: number of values is not a multiple of the number of rows.')
                    OpenCossan.cossanDisp(['Longest  rows: ' num2str( num_header-1+ldx_lo+dlf_sort_idc(end:-1:end-4)', ' %i' ) ' (' num2str( dlf_sort(end:-1:end-4)', ' %i' ) ' characters) '])
                    OpenCossan.cossanDisp(['Shortest rows: ' num2str( num_header-1+ldx_lo+dlf_sort_idc(1:5)'         , ' %i' ) ' (' num2str( dlf_sort(1:5)'         , ' %i' ) ' characters) '])
                end % if
...
if strcmpi(readmode,'line') || ~isempty(errmsg) 
    ...
    if infolvl >= 2 && ~strcmpi(readmode,'line')
        OpenCossan.cossanDisp('Due to error')
        OpenCossan.cossanDisp(strrep(['  ' errmsg],char(10),char([10 32 32])))
        OpenCossan.cossanDisp('txt2mat will now try to read line by line...')
    end % if
    ...
    if infolvl >= 2
        OpenCossan.cossanDisp('Row length info:')
        ...
        OpenCossan.cossanDisp(strcatcell(info_ca));
