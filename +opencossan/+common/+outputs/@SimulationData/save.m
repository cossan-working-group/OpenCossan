...
% See Also: http://cossan.cfd.co.uk/wiki/index.php/save@SimulationData
%
...
% structure
Cnames = XsimOut.TableValues.Properties.VariableNames;
Nsamples = height(XsimOut.TableValues);
...
for iname=1:length(Cnames)
    ...
    switch class(XsimOut.TableValues.(Cnames{iname}))
        case {'double','struct'}
            %                 if ~isvector(Cout{1})
            %                     for n=1:length(Cout)
            %                         Cout{n}=Cout{n}(:);
            ...
            %                     end
            %                 end
            Texp.(Cnames{iname}) = XsimOut.TableValues.(Cnames{iname});
            Tsize.(Cnames{iname}) = size(XsimOut.TableValues.(Cnames{iname}));
            %             end
        ...
        otherwise
            % TODO Throw error for unknown data type
    end
end
...
if ~isfield(Texp,'SimOutReserved_Sdescription')
    if ~isempty(XsimOut.Sdescription)
        Texp.SimOutReserved_Sdescription = XsimOut.Sdescription;
    else
        Texp.SimOutReserved_Sdescription = '';
    end
else
    warning('openCOSSAN:output:SimulationData:save', ...
