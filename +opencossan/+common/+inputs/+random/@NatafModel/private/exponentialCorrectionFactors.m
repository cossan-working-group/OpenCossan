function factor = exponentialCorrectionFactors(~, rvi, rvj, rij)
    %EXPONENTIALCORRECTIONFACTORS Returns the seminalytical nataf model correction factors for
    %combinations including an exponential distribution.
    ...
    switch class(rvj)
        ...
        case 'opencossan.common.inputs.random.WeibullRandomVariable'
            factor = 1.147 + 0.145 * rij - 0.271 * rvj.CoV + 0.010 * rij^2 + ...
                0.459 * rvj.CoV^2 - 0.467 * rij * rvj.CoV;
        otherwise
            factor = NaN;
    end
end
