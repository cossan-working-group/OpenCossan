...
assert(all(ismember(cellfun(@class,varargin,'UniformOutput',false),{'opencossan.common.Dataseries'})),...
    'openCOSSAN:Dataseries:cat',...
    'Not all the inputs are of class Dataseries');

st = dbstack;
assert(~strcmp(st(2).name,'container2vars'),'openCOSSAN:Dataseries:cat',...
    'Throwback error for struct2table to have a table with Dataseries correctly created')
