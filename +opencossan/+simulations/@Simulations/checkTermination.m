...
if ~isempty(Xobj.timeout) && Xobj.timeout>0
    timer = OpenCossan.getTimer();
    if timer.delta(Xobj.initialLaptime) > Xobj.timeout
        SexitFlag=['Maximum execution time reached. Enlapsed time ' ...
            num2str(timer.delta(Xobj.initialLaptime)) ...
...
if isa(Xresults,'opencossan.reliability.FailureProbability')
    ...
    if isa(Xobj,'opencossan.simulations.LineSampling') || isa(Xobj,'opencossan.simulations.AdaptiveLineSampling')
...
if isa(Xobj,'opencossan.simulations.SubsetOriginal') || isa(Xobj,'opencossan.simulations.SubsetInfinite')
