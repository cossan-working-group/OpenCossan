...
scatter(Mdata(1:Xobj.initialSamples,1),Mdata(1:Xobj.initialSamples,2),'.')
Clabels{1}='Level 1 (MCS)';

for iLevel=1:NplotLevels-1
    if LconnectChains
        for jc=1:length(Vchains)
            VindexData=Xobj.chainIndices(Vchains(jc),:,iLevel);
            VrejData=ismember(VindexData,Xobj.rejectedSamplesIndices);
        ...
        VindexData=Xobj.chainIndices(:,:,iLevel);
