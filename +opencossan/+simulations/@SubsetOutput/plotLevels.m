...
SyLabel=Xobj.performanceFunctionName;
Smarker='o';
SmarkerRejected='*';
Svisible='on';
Lseeds=false; % show the seeds for each level and the final samples
NplotLevels=length(Xobj.subsetThreshold);
NfontSize=12;
...
%% Check data
Vg=Xobj.getValues('Sname',Xobj.performanceFunctionName);
...
scatter(ones(Xobj.initialSamples,1),Vg(1:Xobj.initialSamples),Smarker);
line([0 NplotLevels+1],[Xobj.subsetThreshold(1) Xobj.subsetThreshold(1)])

for n=2:NplotLevels
    VindexData=Xobj.chainIndices(:,:,n-1);
    MrejData=ismember(VindexData,Xobj.rejectedSamplesIndices);
    
    if Lseeds
        VseedIndex=Xobj.seedsIndices(Xobj.markovchains*(n-2)+1:Xobj.markovchains*(n-1));
        scatter(n*ones(Xobj.markovchains,1)-0.1,Xobj.subsetPerformance(VseedIndex),'.')
    end
    ...
    if Lseeds
        VgIndex=Xobj.initialSamples*(n-1)+1:Xobj.initialSamples*(n);
        scatter(n*ones(Xobj.initialSamples,1)+0.1,Xobj.subsetPerformance(VgIndex),'.')
    end
    
    line([0 NplotLevels+1],[Xobj.subsetThreshold(n) Xobj.subsetThreshold(n)])
end
