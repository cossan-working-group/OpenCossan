...
VboundsRV=[];
for name=Xobj.Xinput.RandomVariableSetNames
    VboundsRV = [VboundsRV;Xobj.Xinput.RandomVariableSets.(name{1}).getBounds()];
end
...
if isempty(Xobj.Xsamples0)
    ...
    assert(length(Xobj.VreferencePoint)==Nrv+Ndv, ...+Niv
        'openCOSSAN:sensitivity:coreFiniteDifferences', ...
        strcat('The length of reference point (%i) must be equal to' , ...
        ' the sum of the number of random variables (%i) and the number',... , the number of interval variables (%i)
        ' of design variables (%i)'), ...
        length(Xobj.VreferencePoint),Nrv,Ndv) %,Niv
...
for irv=1:Nrv
    ...
    if VboundsRV(VindexRV(1,irv),2) >= (MsamplesPhysicalSpace(VindexRV(1,irv),VindexRV(2,irv)) + Xobj.perturbation*VstdRV(VindexRV(2,irv)))
    ...
    elseif VboundsRV(VindexRV(1,irv),1) <= MsamplesPhysicalSpace(VindexRV(1,irv),VindexRV(2,irv)) - Xobj.perturbation*VstdRV(VindexRV(2,irv));
...
for idv=1:Ndv
    if Xobj.Xinput.DesignVariables(idv).upperBound >= (MDVp(idv,VindexDV(idv)) + Xobj.perturbation*MDVp(VindexDV(idv)))
        MDVp(idv,VindexDV(idv))  = MDVp(idv,VindexDV(idv)) + Xobj.perturbation*MDVp(VindexDV(idv));
    elseif Xobj.Xinput.DesignVariables(idv).lowerBound <= MDVp(idv,VindexDV(idv)) - Xobj.perturbation*MDVp(VindexDV(idv));
        MDVp(idv,VindexDV(idv))  = MDVp(idv,VindexDV(idv)) - Xobj.perturbation*MDVp(VindexDV(idv));
        Vbackward(idv)=true;
    else
        warning('openCOSSAN:sensitivity:coreFiniteDifferences',...
...
if Nrv>0 && Ndv>0
    Xsmli=opencossan.common.Samples('MsamplesPhysicalSpace',MsamplesPhysicalSpace, ...
...
elseif Nrv>0 && Ndv==0
    ...
    Xsmli=opencossan.common.Samples('MsamplesdoeDesignVariables',MDVp,'Xinput',Xobj.Xinput);
end
