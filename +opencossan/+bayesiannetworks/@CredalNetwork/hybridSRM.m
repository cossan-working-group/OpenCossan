function NewNodesObj = hybridSRM(CN, varargin)
% HYBRIDSRM method for the class CredalNetwork
% The method allows to compute the hybrid model in
...
%% Process Input
p = inputParser;
p.FunctionName = 'opencossan.bayesiannetworks.CredalNetwork.hybridSRM';
...
NetNames        = CN.NodesNames;
% Reorder input nodes in topological order
InputNodes                  = unique([Nodes,CN.ParentNodes(ismember(CN.NodesNames,intersect(Nodes,CN.ProbabilisticNodes)))]); %nodes2compute+parents of probabilistic nodes in the envelope
...
SizeNodes                   = CN.NodesSize(NodesIndNet);   % Sizes of the nodes of the envelope (topological order)
InputNodes                  = InputNodes(NodesIndex);    % Names of the ME nodes in topological order
[CSdiscrete, disInd]        = intersect(InputNodes,CN.DiscreteNodes,'stable');    % Discrete nodes: contains new children!
...
CnewNodesName = CN.nodes2compute('DiscreteNodes',CSdiscrete,'InputNodes',InputNodes);
Node2compute  = CN.Nodes(ismember(CN.NodesNames,CnewNodesName));
...
if ~LMEnvelopes && CN.Nodes(newNoInd).Lboolean
    Lboolean        = true;
    Vsrm            = [SizeDiscrete(1,1:end-1),1];
    LcomputedOutcome= find(~cellfun(@isempty,CN.CPDs{newNoInd}));
...
%     end
    ...
    if Lboolean && (any(cellfun(@isempty,CN.CPDs{newNoInd}(:))))
        combination{end} = find(~cellfun(@isempty,CN.CPDs{newNoInd})); 
    elseif Lboolean && ~(any(cellfun(@isempty,CN.CPDs{newNoInd}(:))))
        combination{end} = 1; 
    end
    
    % Prepare input
    Xinput= CN.hybridInput('Combination',combination,...
    ...
    for inew=1:length(CnewNodesName) 
        ...
        IndDisPA(IndDisPA==0)=[];
        [~,indexdiscreteparents]    = intersect(CSdiscrete,[CN.Nodes(newNoInd(inew)).Parents,CnewNodesName(inew)],'stable');
        ...
        if length(CPDSizeDiscreteparents)==1 %nondiscrete parents
            ...
            CPD                     = reshape(CN.Nodes(newNoInd(inew)).CPD,CPDSizeDiscreteparents);
            CSscript(1,2*inew)      = CPD(combination{newChInd(inew)});
        elseif all(CPDSizeDiscreteparents==1)
            CPD                     = reshape(CN.Nodes(newNoInd(inew)).CPD,[CPDSizeDiscreteparents,CN.Nodes(newNoInd(inew)).Size]);
            CSscript(1,2*inew)      = CPD(combination{[IndDisPA,newChInd(inew)]});
        else
            CPD                     = reshape(CN.Nodes(newNoInd(inew)).CPD,CPDSizeDiscreteparents); % N.B. size probabilistic nodes always one!
            CSscript(1,2*inew)      = CPD(combination{[IndDisPA,newChInd(inew)]});
        end   
    end
   ...
   %% COMPUTE FAILURE PROBABILITY
    if isa(XsimParfor,'opencossan.simulations.AdaptiveLineSampling') && ~isempty(CN.Nodes(newNoInd).MimportantDirection)
        [~,~,indInp ]=intersect([Xinput.CnamesRandomVariable,Xinput.CnamesIntervalVariable],...
                lower(setdiff(CN.Nodes(newNoInd(inew)).Parents,CN.DiscreteNodes,'stable')),'stable');
            VimportantDirInNode=CN.Nodes(newNoInd(inew)).MimportantDirection(combination{newChInd(inew)},:);
            XsimParfor.Vdirection=VimportantDirInNode(indInp);        
    elseif isa(XsimParfor,'opencossan.simulations.AdaptiveLineSampling') && isempty(CN.Nodes(newNoInd).MimportantDirection)
...
NewNodesObj = CN.buildNewImpreciseNodes(CnewNodesName, CPTtotal, CSdiscrete, SizeDiscrete);
