function Vbounds = probabilisticNodeBounds(Node, varargin)
...
p = inputParser;
p.FunctionName = 'opencossan.bayesiannetworks.ProbabilisticNode.probabilisticNodeBounds';
...
for ivar=1:Nvs
    ...
    if ~isinf(XVs.lowerBound) % N.B. this condition is always respected for BVs
        
        VlowerBound(ivar)=XVs.lowerBound;
        
    else
        
        VlowerBound(ivar)=XVs.cdf2physical(1E-10);%1E-10
        
    end
    
    % collect upper bound of the RV/BV
    if ~isinf(XVs.upperBound)  % N.B. this condition is always respected for IVs
        
        VupperBound(ivar)=XVs.upperBound;
        
    else
        
        VupperBound(ivar)=XVs.cdf2physical(1-(1E-10));%(1E-10)
