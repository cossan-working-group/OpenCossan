function Toutput = TrussMaxDisp(Tinput)
%% TRUSSMAXDISP - Main MIO code
% for each sample
for isample = 1:length(Tinput),
    ...
    % compute the beam lemgth
    beamLengths = getBeamLength(D);
...
function D=DataTruss(Tinput)
%% DATATRUSS - Construct the truss structure struct given the structural
% parameters
...
for ibeam=1:25
    eval(['E' num2str(ibeam) '= Tinput.E_' num2str(ibeam) ';']);
end
...
function [F,U,R]=ST(D)
%% ST Compute stresses of the structure
...
%% GETBEAMLENGTH Compute the length of beams
