...
% Define inputs
Szero=common.inputs.Parameter('description','Number of susceptible individuals','value',1000);
gamma=common.inputs.RandomVariable('Sdescription','Infection coefficient', ...
    'Sdistribution','uniform','lowerbound',0,'upperbound',1);
kappa=common.inputs.RandomVariable('Sdescription','Contact coefficient', ...
    'Sdistribution','beta','par1',2,'par2',7);
r=common.inputs.RandomVariable('Sdescription','Recovery rate', ...
    'Sdistribution','uniform','lowerbound',0,'upperbound',1);
delta=common.inputs.RandomVariable('Sdescription','Death rate', ...
    'Sdistribution','uniform','lowerbound',0,'upperbound',1);

% Now we group all the inputs in a Input object
Xrvset=common.inputs.RandomVariableSet('Cmembers',{'gamma' 'kappa' 'r' 'delta'}, ...
    'CXmembers',{gamma kappa r delta});

Xin=common.inputs.Input('Sdescription','Inputs of the infective process', ...
    'XrandomVariableSet',Xrvset,'Xparameter',Szero);

% Define the physical model
% Creating a matlab script for computing Y=gamma*kappa*S0-r-delta
Xm=workers.Mio('Sdescription','Infection model early stage', ... 
       'Cinputnames',{'Szero' 'gamma' 'kappa'  'r' 'delta'}, ...
       'Coutputnames',{'Y'},...
       'Sfile','infectionMatrix.m','Spath',Sfolder,...
       'Lfunction',false,'Sformat','matrix');
   
Xev=workers.Evaluator('Xmio',Xm);
Xmdl=common.Model('Xinput',Xin,'Xevaluator',Xev,...
