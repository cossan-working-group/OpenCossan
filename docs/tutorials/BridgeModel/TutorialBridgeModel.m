...
h=common.inputs.RandomVariable('Sdistribution','normal','mean',0.001,'cov',0.05,'lowerBound',0);

hSet=common.inputs.RandomVariableSet('Xrv',h,'Nrviid',20);
% Please note that the random variables in the randomvariables set are named h_1
% to h_20
disp(hSet.Cmembers)

%% Beams geometry: E modulus
E=common.inputs.RandomVariable('Sdistribution','normal','mean',210e9,'cov',0.03,'lowerBound',0);

ESet=common.inputs.RandomVariableSet('Xrv',E,'Nrviid',20);
% Please note that the random variables in the random variables set are named E_1
% to E_20
disp(ESet.Cmembers)

%% Beams geometry: length
l=common.inputs.RandomVariable('Sdistribution','normal','mean',0.36,'cov',0.05,'lowerBound',0);

lSet=common.inputs.RandomVariableSet('Xrv',l,'Nrviid',20);
% Please note that the random variables in the random variables set are named l_1
% to l_20
disp(lSet.Cmembers)

%% Supports: stiffness
k=common.inputs.RandomVariable('Sdistribution','normal','mean',200,'cov',0.10,'lowerBound',0);

kSet=common.inputs.RandomVariableSet('Xrv',k,'Nrviid',21);
% Please note that the random variables in the random variables set are named k_1
% to k_21
disp(kSet.Cmembers)

%% Supports: rotation stiffness
w=common.inputs.RandomVariable('Sdistribution','normal','mean',40,'cov',0.16,'lowerBound',0);

wSet=common.inputs.RandomVariableSet('Xrv',w,'Nrviid',21);
% Please note that the random variables in the random variables set are named w_1
% to w_21
disp(kSet.Cmembers)

%% Supports: damping
c=common.inputs.RandomVariable('Sdistribution','normal','mean',0.4,'cov',0.25,'lowerBound',0);

cSet=common.inputs.RandomVariableSet('Xrv',c,'Nrviid',21);
% Please note that the random variables in the random variables set are named c_1
% to c_21
disp(cSet.Cmembers)

%% Load
load=common.inputs.Parameter('description','Harmonic load frequency (Hz)','value',10);
rho=common.inputs.Parameter('description','material density','value',7800);
width=common.inputs.Parameter('description','BeamWidth','value',0.04);

%% Create a Input object
Xinput=common.inputs.Input('CXmembers',{hSet lSet ESet kSet wSet cSet load rho width},...
...
Xmio=workers.Mio('Spath',Sfolder,'Sfile','bridgeModel.m',...
         'Sformat','matrix',...
...
XmodelBridge=common.Model('Xinput',Xinput,'Xevaluator',Xevaluator);
