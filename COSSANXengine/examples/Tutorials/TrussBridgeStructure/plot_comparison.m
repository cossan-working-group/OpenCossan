...
for isim = 1:Nsamples_validation
    ...
    Tfrf_mm{isim} = frf(Xm_mm,'Sfrftype','acc','Vforce',Vforce','Vexcitationfrequency',Vfreqrange,'Vdofs',DOF_obs,'Vzeta',Vzeta);
    
    Xm_fm = Xmm.XvalidationOutput(isim);
    Vzeta = modal_damping_ratio*ones(Nmodes_fm,1);
    Vforce = Xm_fm.MPhi'*Vf;
    Tfrf_fm{isim} = frf(Xm_fm,'Sfrftype','acc','Vforce',Vforce','Vexcitationfrequency',Vfreqrange,'Vdofs',DOF_obs,'Vzeta',Vzeta);
