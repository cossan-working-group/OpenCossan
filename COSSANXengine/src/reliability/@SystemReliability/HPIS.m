...
% HPIS estimates the failure probability for parallel
% systems
%
...
for imcs=1:length(Cmcs)
    ...
    % Create a structure with Input
    Xs=Samples('Msamplesstandardnormalspace',M12,'Xinput',Xin);
    Xin.Xsamples=Xs;
    Tvalues=Xin.getStructure;
    XsimData(imcs)=SimulationData('Tvalues',Tvalues); %#ok<AGROW>
