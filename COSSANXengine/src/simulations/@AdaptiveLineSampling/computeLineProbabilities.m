...
updateCounter=0;
for iLine=1:Nlines
    updateCounter=VdirectionalUpdate(iLine)+updateCounter;
    % The pf is estimated averaging over the following number of lines
    % NpfLines=Nlines - NeffectiveUpdates
    if updateCounter>=Xobj.NeffectiveUpdates       % if NeffectiveUpdates==0 consider all lines
        % state flag
