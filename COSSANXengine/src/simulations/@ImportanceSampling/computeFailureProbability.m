...
%% IS simulation
SexitFlag=[]; % Set Default Exit flag
while isempty(SexitFlag)
    
    %update counter of batches
    ...
    %% evaluate Xtarget object
    XsimOut = apply(Xtarget,Xinput);
    
    %% Export the weights
    XsimOut=XsimOut.addVariable('Cnames',{Xobj.SweightsName},'Mvalues',Xs.Vweights);
    
    %% Compute Pf
    if Xobj.ibatch==1
        % Initialize FailureProbability object
        Xpf = FailureProbability('CXmembers',{Xtarget}, ...
            'Smethod','ImportanceSampling','SweigthsName',Xobj.SweightsName,...
            'XsimulationData',XsimOut);
    else
...
OpenCossan.setLaptime('Sdescription','End pf@ImportanceSampling');
