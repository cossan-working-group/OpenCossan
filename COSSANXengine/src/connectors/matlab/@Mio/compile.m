...
fprintf(Nfid, ' %% MIO wrapper generated by the method @mio/compile \n');
fprintf(Nfid, [' %% on ' datestr(now) '\n'] );
fprintf(Nfid, ' %% Load input from file \n');
fprintf(Nfid, ' load mioINPUT.mat; \n');
if Xm.Lfunction % the mio is a function
    fprintf(Nfid, ' %% call function \n');
    if Xm.Liostructure
        fprintf(Nfid, [' Toutput = ' Xm.Sfile '(Tinput); \n']);
    else
        fprintf(Nfid, [' Moutput = ' Xm.Sfile '(Minput); \n']);
    end
else % the mio is a script
    fprintf(Nfid, ' %% call script \n');
    if ~isempty(Xm.Sscript) % the script is in the field Sscript
        fprintf(Nfid, [Xm.Sscript '\n']);
    else % the script is in a file
        % copy the content of the script file in mioWrapper.m
        Nfid_script = fopen([fullfile(Xm.Spath,Xm.Sfile) '.m'],'r');
        line_id = 0; 
        while 1
