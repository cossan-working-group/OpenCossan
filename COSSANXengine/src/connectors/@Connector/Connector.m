classdef Connector
    ...
    properties (Dependent=true)
        Cinputnames   % Names of the extracted values
        Coutputnames   % Names of the extracted values
    ...
    methods
        ...
        function Xobj = Connector(varargin)
            ...
            if exist('SpredefinedType','var')
                ...
                try
                    SconnectorFolder=fileparts(which('Connector.m'));
                    run(fullfile(SconnectorFolder,'predefinedConnectors',SpredefinedType));
                catch me
                    error('openCOSSAN:Connector:Connector',...
        ...
        function Linjectors=get.Linjectors(Xobj)
            ...
            for n=1:length(Xobj.CXmembers)
                Linjectors(n) = isa(Xobj.CXmembers{n},'Injector');
            end
        end
        ...
        function Lextractors=get.Lextractors(Xobj)
            ...
            for n=1:length(Xobj.CXmembers)
                Lextractors(n) = isa(Xobj.CXmembers{n},'Extractor');
            end
        end
    ...
    methods (Access=private)
        ...
        function checkFiles(Xobj) 
            ...
            assert(~isempty(Xobj.Smaininputpath),'openCOSSAN:Connector:checkFiles',...
                'Please define a main input path.')
