classdef Samples
    ...
    methods
        ...
        display(Xobj)  % shows the summary of the Samples object
        ...
        function Xobj     = Samples(varargin)
                ...
                switch lower(varargin{k})
                    ...
                    case {'cdoedesignvariables','cxdoedesignvariables','cdv'}
                        for imem=1:length(varargin{k+1})
                            if isa(varargin{k+1}{imem},'DesignVariable')
                    ...
                    case {'tsamplesphysicalspace'}
                        % store names of the fields
                        Cnames  = fieldnames(varargin{k+1})';
                        MX=cell2mat(squeeze(struct2cell(varargin{k+1})))';
                    case {'tsamplesstandardnormalspace'}
                        % store names of the fields
                        Cnames  = fieldnames(varargin{k+1})';
                        MU=cell2mat(squeeze(struct2cell(varargin{k+1})))';
                    ...
                    case 'xinput'
                        ...
                        Xobj.Xrvset  = struct2cell(varargin{k+1}.Xrvset);
                        Xobj.Xbset   = struct2cell(varargin{k+1}.Xbset);
                        Xobj.XdesignVariable  = struct2cell(varargin{k+1}.XdesignVariable);
                        Xobj.XstochasticProcess = struct2cell(varargin{k+1}.Xsp);
                        Xobj.CnamesStochasticProcess = varargin{k+1}.CnamesStochasticProcess;
                        Xobj.CnamesDesignVariables= varargin{k+1}.CnamesDesignVariable;
                        Xobj.CnamesRandomVariableSet= varargin{k+1}.CnamesRandomVariableSet;
                        Xobj.CnamesBoundedSet  = varargin{k+1}.CnamesBoundedSet;
                        
                    case{'xrvset','xrandomvariableset','xgrvset','xgaussianrandomvariableset'},
                        %  Case of RandomVariableSet object
                        if isa(varargin{k+1}(1),'RandomVariableSet') || isa(varargin{k+1}(1),'GaussianMixtureRandomVariableSet')
                    ...
                    case{'cxrvset','cxrandomvariableset','cxgrvset','cxgaussianrandomvariableset'},
                        %  Case of RandomVariableSet object
                        for imem=1:length(varargin{k+1})
                            if isa(varargin{k+1}{imem},'RandomVariableSet') || isa(varargin{k+1}{imem},'GaussianMixtureRandomVariableSet')
                    ...
                    case 'xstochasticprocess'
                        ...
                        if isa(varargin{k+1}(1),'StochasticProcess')
                    ...
                    case{'cxstochasticprocess'},
                        %  Case of RandomVariableSet object
                        for imem=1:length(varargin{k+1})
                            if isa(varargin{k+1}{imem},'StochasticProcess')
                    ...
                    case 'cxdataseries'
                        ...
                        if all(cellfun(@(x)(isa(x,'Dataseries')),varargin{k+1}))
                            Xobj.Xdataseries...
                    ...
                    case 'xdataseries'
                        %Case of a dataseries
                        if (isa(varargin{k+1},'Dataseries'))
            ...
            %% Mapping the samples in the UnitHyperCube
            if exist('MX','var')
            ...
            if ~isempty(Xobj.Xdataseries)
                Vsamples = [Vsamples size(Xobj.Xdataseries,1)];
            end
            ...
            if ~isempty(Vsamples)
                assert(all(Vsamples==Vsamples(1)),...
                    ...
                    'Samples BoundedSet        : %i'],...
                    size(Xobj.MsamplesHyperCube,1),...
                    size(Xobj.Xdataseries,1),...
                    size(Xobj.MdoeDesignVariables,1),...
                    size(Xobj.MsamplesEpistemicSpace));
        ...
        function CnamesRandomVariable = get.CnamesRandomVariable(Xobj)
            ...
            for n=1:length(Xobj.Xrvset),
                CnamesRandomVariable=[CnamesRandomVariable Xobj.Xrvset{n}.Cmembers]; %#ok<AGROW>
            end
        ...
        function Xobj = set.MsamplesPhysicalSpace(Xobj,MsamplesPhysicalSpace)
            ...
            Xobj.MsamplesHyperCube=zeros(size(MsamplesPhysicalSpace));
        ...
        function Tsamples = get.Tsamples(Xobj)
            ...
            Crvvalue =num2cell(Xobj.MsamplesPhysicalSpace);
            % Extract dataseries
            Cspvalue = num2cell(Xobj.Xdataseries);
            % Extract values of interval variables
            CitnervalValue = num2cell(Xobj.MsamplesEpistemicSpace);
            % Construct the structure
            Tsamples=cell2struct([Crvvalue Cspvalue CdesignVariableValue CitnervalValue],...
                Xobj.Cvariables, 2);
        end
        ...
        function Xobj = set.Tsamples(Xobj,Tsamples)
            ...
            CrequiresFields = [Xobj.CnamesRandomVariable,Xobj.CnamesDesignVariables];
            ...
            Xobj.MsamplesPhysicalSpace = MfullSamples(:,VlocRV);
