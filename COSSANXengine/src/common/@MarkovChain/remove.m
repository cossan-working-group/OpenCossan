...
if exist('Vchain','var')
    ...
    Mreplicated=Xmkv.Xsamples(end-Nremove).MsamplesHyperCube(Vchain,:);
    
    for ichain=0:Nremove-1
        MHCubeSamples=Xmkv.Xsamples(end-ichain).MsamplesHyperCube; %Retrive Samples object
        MHCubeSamples(Vchain,:) = Mreplicated;
        % Modify the samples 
        Xmkv.Xsamples(end-ichain).MsamplesHyperCube=MHCubeSamples;
    end
else
