...
if isempty(Xobj.VreferencePoint)
    ...
    VreferencePointUserDefinedIV=zeros(1,length(CnamesIV));
    ...
    for n=1:length(CnamesIV);
        VreferencePointUserDefinedIV(n)=Tdefault.(CnamesIV{n});
    end

    Xobj.VreferencePoint=[VreferencePointUserDefinedRV VreferencePointUserDefinedDV VreferencePointUserDefinedIV];
end

if isempty(Xobj.Cinputnames)
   Xobj.Cinputnames=Xobj.Xinput.CnamesRandomVariable;
else
   assert(all(ismember(Xobj.Cinputnames,[Xobj.Xinput.CnamesRandomVariable, Xobj.Xinput.CnamesIntervalVariable, Xobj.Xinput.CnamesDesignVariable])), ...
