...
if exist('stdErrorTotalLow','var') || exist('stdErrorUpperLow','var') || exist('stdErrorUpperLow','var')
    if Lplotcov
        Clegend{end+1} = 'error bars (+/- 1 std)';
    else
        Clegend{end+1} = ['Confidence Intervals: ' sprintf('%3.2f%% %3.2f%%',Xobj.Valpha)];
    end
end
...
legend(gca,Clegend)
box(gca,'on')
figHandle.CurrentAxes.YLim(1)=0;
set(hbar(1),...
