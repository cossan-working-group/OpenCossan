                ...
                switch lower(varargin{k})
                    ...
                    case {'csmembers'} % Pass the object by names
                        for imem=1:length(varargin{k+1})
                            ...
                            if isa(Xmem,'ProbabilisticModel')
                            ...
                            elseif isa(Xmem,'SimulationData')
                                Xoutput=Xmem;
                            else
                                mc=metaclass(Xmem);
                                if strcmp(mc.SuperClasses{1}.Name,'Simulations')
                        ...
                        for n=1:length(varargin{k+1})
                            switch class(varargin{k+1}{n});
                                case 'ProbabilisticModel'
                                    Xobj.SperformanceFunction=varargin{k+1}{n}.XperformanceFunction.Soutputname;
                                    Xobj.stdDeviationIndicatorFunction=varargin{k+1}{n}.XperformanceFunction.stdDeviationIndicatorFunction;
                                case 'PerformanceFunction'
                                    Xobj.SperformanceFunction=varargin{k+1}{n}.Soutputname;
                                    Xobj.stdDeviationIndicatorFunction=varargin{k+1}{n}.stdDeviationIndicatorFunction;
                                case 'SimulationData'
                                    Xoutput=varargin{k+1}{n};
                                otherwise
                                    mc=metaclass(varargin{k+1}{n});
                                    if strcmp(mc.SuperClasses{1}.Name,'Simulations')
                                        Xobj.Smethod=mc.Name;
                                    else
                                        error('openCOSSAN:output:FailureProbability',...
